    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('order_date') ? 'is-invalid' : '' }}" type="text" name="order_date" id="order_date" value="{{ old('order_date', $order->order_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('distributor') ? 'is-invalid' : '' }}" name="distributor_id" id="distributor_id" required>
                    @foreach($distributors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('distributor_id') ? old('distributor_id') : $order->distributor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
