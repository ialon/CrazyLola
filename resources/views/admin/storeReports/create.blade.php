...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.storeReport.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.store-reports.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="store">{{ trans('cruds.storeReport.fields.store') }}</label>
                <input class="form-control {{ $errors->has('store') ? 'is-invalid' : '' }}" type="text" name="store" id="store" value="{{ old('store', '') }}" required>
                @if($errors->has('store'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('products') ? 'is-invalid' : '' }}" type="number" name="products" id="products" value="{{ old('products', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orders') ? 'is-invalid' : '' }}" type="number" name="orders" id="orders" value="{{ old('orders', '') }}" step="1">
