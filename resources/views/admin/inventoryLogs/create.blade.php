...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.inventoryLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.inventory-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="inventory">{{ trans('cruds.inventoryLog.fields.inventory') }}</label>
                <input class="form-control {{ $errors->has('inventory') ? 'is-invalid' : '' }}" type="number" name="inventory" id="inventory" value="{{ old('inventory', '') }}" step="1" required>
